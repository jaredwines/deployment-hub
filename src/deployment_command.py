...
class DeploymentCommand(ABC): 
    ...
    def _make_dir(self, *target_dirs):
        for target_dir in target_dirs: 
            stdout = self.__ssh_deployment_client._exec_command("if [ -d " + target_dir + " ]; then echo 'True'; else echo 'False'; fi")
    ...
    def _clone_git_repo(self, target_dir = None): 
        ...
        self.__ssh_deployment_client.exec_command("git clone -b " + self.__branch + " " + self.__git_repo + " " + target_dir)
